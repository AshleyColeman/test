...
module.exports = {
   ...
   */
     ...
     */
		...
		/**
     * get list transaction by id,year
     *
     * @returns
     */
		chartByYear: {
			...
			rest: {
				method: "GET",
				path: "/chart/:id/transaction/:year",
			},
			async handler(ctx) {
				const { user } = ctx.meta;
		...
		/**
     * get all transaction by walletId
     *
     * @returns
     */
		chartAllByWallet: {
			...
			async handler(ctx) {
				...
				console.log("---all trans wallet---", total);
				return success({
					result: {
						total: total,
					},
